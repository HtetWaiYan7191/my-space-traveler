...
function MissionCard ({ mission }){
  ...
  <tr className="table-row flex items-start">
    ...
    <th className=" w-[15%]">
        <button type="button" className={isJoined ? `border px-2 py-1 rounded-md border-red-500 text-red-500` : ` px-1 py-1 border border-slate-700 rounded-md text-gray-500`} onClick={handleClick}>{isJoined ? 'Leave Meeting ' : 'Join Meeting'}</button>
    </th>
  </tr>
)};
